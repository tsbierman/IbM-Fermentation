...
def generate_gif(args: Dict):
    ...
    with h5py.File(f'./{directory}/results1D.jld2', 'r') as f:
        print('Loading results file...')
        print(f["bac_saved_vecint"])
        bac = {}
        for sub_struct in ['bac_saved_vecint', 'bac_saved_matfloat', 'bac_saved_matint', 'bac_saved_matbool']:
            for k in f[sub_struct].keys():
                bac[k] = np.array(f[sub_struct][k]).squeeze()
...
if args.finished:
    simulation_file = f'./{directory}/sim_{sim}.jld2'
else:
    simulation_file = f'./sim_{sim}.jld2'
