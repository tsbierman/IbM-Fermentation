function smoothing(phi, rhs, L_lhs)
    ...
    Arguments
    phi:            The matrix of what has to be smoothed (u in the equation). It can be either concentrations or the error that will be smoothed
    rhs:            The right hand side of the equation (f in the equation above). It can be either the rhs of the diffusion-reaction or residuals
    ...
    L_sm = copy(L_lhs) # To prevent L_lhs from changing
    L_sm[2,2] = 0
