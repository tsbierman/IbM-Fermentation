function V_Cycle!(phi, diffRegion, bulk_value, f, L_0, L_restriction, L_prolongation, maxDepth, depth, iter_pre, iter_post, iter_final)
    ...
    phi:                The matrix x in Ax=b. It is either concentration (phi) or error (e)
    diffRegion:         A BitMatrix indicating per gridcell whether that cell is in the diffusion region
    bulk_value:         The boundary value of the compound outside of the diffusion region, thus the value in the bulk/dirichlet boundary condition
    f:                  The matrix b in Ax = b. It is either rhs of diffusion-reaction equation, or residuals after residuals restriction
    ...
    maxDepth:           The minimum number of gridcells in each direction in the highest recursion of the V-cycle, 
    ...
    iter_post:          The number of smoothing steps after error correction
    ...
    eps = zeros(size(rhs))                                          # Initial guess is zeros as when all errors are 0 we have perfect solution
    ...
    # Prolongation of eps and correction of phi
