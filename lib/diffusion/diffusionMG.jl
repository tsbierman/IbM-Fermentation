function calculate_rhs_dirichlet(phi, L_rhs, value, diffRegion)
    """
    This function calculates part of the rhs of the diffusion-reaction equation using convolution on 1 compound only.
    Only on the diffusion region. It assumes that there is at least one gridcell of bulk liquid around the diffusion region.

    Arguments
    phi:            A matrix with starting values for 1 compound
    L_rhs:          The kernel used for the rhs convolution
...
function diffusionMG!(conc, reaction_matrix, bulk_concentrations, diffRegion, grid_float, constants_float, constants_vecfloat, constants_vecstring, Time)
    """
    This function solves diffusion for all molecules in the liquid phase using the multigrid method.
    ...
    reaction_matrix:        A (ny, nx, ncompounds) matrix with per gridcell and per compound the change [mol/L/h] due to bacterial activity
    ...
    Returns
    conc:                   A (ny, nx, ncompounds) matrix containing the concentrations after solving the diffusion equations [mol/L]
    """
    ...
    # Set smoothing iterations for V_cycle
    iter_pre = 6
    iter_post = 7
    iter_final = 5
    ...
    for icompound in 1:nCompounds
        ...
        # Check for negative values, raise error if found
