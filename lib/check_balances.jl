function check_balances(bac_vecfloat, bac_vecint, bac_vecbool, constants_float, constants_vecfloat, constants_vecint, constants_vecstring, constants_vecbool, constants_matfloat, settings_string, reaction_matrix, bulk_concentrations, invHRT, bulk_change, tolerance)
    ...
    settings_string:        A "String" struct simulation settings of type String
    ...
    for specie in 1:length(constants_vecstring.speciesNames)
        ...
        # Update estimate
        estimated_compounds = estimated_compounds .+ compoundChange_growth .- compoundChange_decay # Update estimated compounds, minus compound_decay due to it begin negative        
    end
    ...
    difference = actual_compoundChange .- estimated_compounds   # Close balance
    biomass_closes = maximum(abs.(difference)) <= tolerance     # Check

    # Select values
    influent = constants_vecfloat.influent_concentrations
    outflow = bulk_concentrations

    # Calculate HCO3- concentration in liquid
    ...
    # Only look at specific CO2 specie
